export default function PriceDetailsCard({ price, coupon, format }) {
  return (
    <div className="rounded-xl border bg-white p-4">
      ...
      <Row
        label={
          <span className="inline-flex items-center gap-1">
            Platform Fee <span className="cursor-help text-xs text-gray-400">i</span>
          </span>
        }
      ...
      <Row label={<span className="font-semibold">Total Amount</span>} value={<span className="font-semibold">{format(price.grandTotal)}</span>} />
    </div>
  );
}
